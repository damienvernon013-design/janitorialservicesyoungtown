...
//
// CRM_API_TOKEN is shared across every site in the portfolio, and the
// PushLead schema has no dedicated site-id field, so SITE_SOURCE_TAG is
// prepended into customer.notes on every submission to keep leads from
// this domain traceable back to it in the CRM.

const CRM_ENDPOINT = 'https://thequotemasters.com/crm_api/api.php?action=push_lead';
const MAX_FIELD_LENGTH = 2000;
const ZIP_DEFAULT = '85363'; // Youngtown, AZ
const ADDRESS_DEFAULT = 'Youngtown, AZ';
const SITE_SOURCE_TAG = 'Site: janitorialservicesyoungtown.com';
...
function buildWizardPayload(body) {
  ...
  const notes = [SITE_SOURCE_TAG, clean(input.notes)].filter(Boolean).join(' | ');
...
module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', 'https://janitorialservicesyoungtown.com');
